...
export default function KalenderPage() {
  ...
  useEffect(() => {
    fetch("/api/checkin/dates")
      .then((res) => (res.ok ? res.json() : null))
      .then((data) => {
        if (Array.isArray(data)) setCheckinDates(data.filter((x) => typeof x === "string"));
        else if (Array.isArray((data as any)?.dates)) setCheckinDates((data as any).dates.filter((x: unknown) => typeof x === "string"));
        else setCheckinDates([]);
      })
  ...
  const selectedHasCheckin = selectedDate ? checkinDates.includes(selectedDate) : false;
